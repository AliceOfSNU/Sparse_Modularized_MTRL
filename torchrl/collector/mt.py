...
class MultiTaskCollectorWithRouting(BaseCollector):
    def __init__(self, env, pf, pfr, replay_buffer, # required arguments for base
            env_cls, env_args, # required arguments for multi-task
            progress_alpha=0.1, reset_idx=False, # optional named
            **kwargs):
        
        super().__init__(env, pf, replay_buffer, pfr = pfr, **kwargs) # collector base
        self.env_cls  = env_cls
        self.env_args = env_args
        self.tasks = list(self.env_cls.keys())
        self.task_nums = len(self.tasks)
    ...
    def take_actions(cls, funcs, env_info, ob_info, replay_buffer):
        ...
        task_idx = env_info.env_rank
        # sample action without grad
        # no grad because we are only collecting experience, not training anything..
        with torch.no_grad():
            embedding_input = torch.zeros(env_info.task_nums)
            embedding_input[task_idx] = 1
            embedding_input = embedding_input.unsqueeze(0)
            obs_input =  torch.Tensor(ob).to(env_info.device).unsqueeze(0)
            # pass through router
            out = pfr.explore(obs_input, embedding_input.to(env_info.device))
            route_act = out["action"]
            # add pass through modularized network, provide the routes.
            out = pf.explore(obs_input, out["routes"], out["final_route"])
            act = out["action"]
        # the buffer and env requires numpy
        act = act.detach().cpu().numpy()
        route_act = route_act.detach().cpu().numpy()
        if not env_info.continuous:
            act = act[0]
        route_act = route_act[0]
        ...
        sample_dict = {
            "obs": ob,
            "next_obs": next_ob,
            "acts": act, #numpy array
            "routing_acts": route_act, #numpy array containing concatenations of one-hot selections
            "task_idxs": [task_idx],
            "rewards": [reward],
            "terminals": [done]
        }
        ...
        for task_idx, env in enumerate(self.eval_envs):
            ...
            for idx in range(self.eval_episodes):
                ...
                while not done: # one episode
                    ...
                    embedding_input[task_idx] = 1
                    embedding_input = embedding_input.unsqueeze(0).to(self.device)
                    route = self.pfr.eval_act(torch.Tensor( eval_ob ).to(self.device).unsqueeze(0), embedding_input)
                    act, stat_info = self.pf.eval_act( torch.Tensor( eval_ob ).to(self.device).unsqueeze(0), route, return_weights = True)
